...
def komb(seq, start,seq2):
    for mystr in seq:
        for mystr1 in start:
            for mystr2 in seq2:
                yield mystr + mystr1 + mystr2
...
def print_to_file(filename, unique_list_finished):
    ...
    inspect = "n"#input("> Hyperspeed Print? (Y/n) : ").lower()
    if inspect == "y":
        try:
...
def interactive():
    ...
    with open("Names_Dataset.txt", "r") as file:
        lines = file.readlines()
    with open("born_year.txt", "r") as years:
        yearss = years.readlines()
    with open("common_words.txt","r") as comwords:
        wordss = comwords.readlines()
    profile = {}

    name = random.choice(lines)#input("> First Name: ").lower()
    name = name.strip()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] You must enter a name at least!")
        name = random.choice(lines)#input("> Name: ").lower()
    profile["name"] = str(name)
    surname= random.choice(lines)#input("> Surname: ").lower()
    surname = surname.strip()
    profile["surname"] = str(surname)
    nick= random.choice(lines)#input("> Nickname: ").lower()
    nick = nick.strip()
    profile["nick"] = str(nick)
    birthdate =random.choice(yearss) #input("> Birthdate (DDMMYYYY): ")
    birthdate=birthdate.strip()
    while len(birthdate) != 0 and len(birthdate) != 4:
        print("\r\n[-] You must enter 8 digits for birthday!")
        birthdate = str(birthdate)#input("> Birthdate (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")
    partner = random.choice(lines)#input("> Partners) name: ").lower()
    partner = partner.strip()
    profile["wife"] = str(partner)
    partner_nick = random.choice(lines)#input("> Partners) nickname: ").lower()
    partner_nick = partner_nick.strip()
    profile["wifen"] = str(partner_nick)
    wifeb = random.choice(yearss) #nput("> Partners) birthdate (DDMMYYYY): ")
    wifeb = wifeb.strip()
    while len(wifeb) != 0 and len(wifeb) != 4:
        print("\r\n[-] You must enter 8 digits for birthday!")
        wifeb = input("> Partners birthdate (DDMMYYYY): ")
    profile["wifeb"] = str(wifeb)
    print("\r\n")
    kids = random.choice(lines)#input("> Child's name: ").lower()
    kids = kids.strip()
    profile["kid"] = str(kids)
    kids_nick = random.choice(lines)#input("> Child's nickname: ").lower()
    kids_nick= kids_nick.strip()
    profile["kidn"] = str(kids_nick)
    kidb = random.choice(yearss)#input("> Child's birthdate (DDMMYYYY): ")
    kidb= kidb.strip()
    while len(kidb) != 0 and len(kidb) != 4:
        print("\r\n[-] You must enter 8 digits for birthday!")
        kidb = str(kidb)#input("> Child's birthdate (DDMMYYYY): ")
    profile["kidb"] = str(kidb)
    print("\r\n")
    pet = random.choice(lines)#input("> Pet's name: ").lower()
    pet = pet.strip()
    profile["pet"] = str(pet)
    companyy= random.choice(lines)#input("> Company name: ").lower()
    companyy= companyy.strip()
    profile["company"] = str(companyy)
    print("\r\n")
    ...
    profile["spechars1"] = "n" #input(
        #"> Do you want to add special chars at the end of words? Y/[N]: "
    #).lower()

    profile["randnum"] = "n" #input(
        #"> Do you want to add some random numbers at the end of words? Y/[N]:"
    #).lower()
    profile["leetmode"] = "n"#input("> Leet mode? (i.e. leet = 1337) Y/[N]: ").lower()
...
def generate_wordlist_from_profile(profile):
    ...
    kombi = {}
    kombi[1] = list(komb(kombinaa, bdss,kombinaaw))
    kombi[1] += list(komb(kombinaa, bdss,kombinaak ))
    kombi[2] = list(komb(kombinaaw, wbdss,kombinaak))
    kombi[2] += list(komb(kombinaaw, wbdss,kombinaaw))
    kombi[3] = list(komb(kombinaak, kbdss,word))
    kombi[3] += list(komb(kombinaak, kbdss,kombinaac))
    kombi[4] = list(komb(kombinaa, years,kombinaaw))
    kombi[4] += list(komb(kombinaa, years,word))
    kombi[5] = list(komb(kombinaac, years,kombinaak))
    kombi[5] += list(komb(kombinaac, years,word))
    kombi[6] = list(komb(kombinaaw, years,kombinaak))
    kombi[6] += list(komb(kombinaaw, years,kombinaac))
    kombi[7] = list(komb(kombinaak, years,kombinaaw))
    kombi[7] += list(komb(kombinaak, years,kombinaac))
    kombi[8] = list(komb(word, bdss,kombinaaw))
    kombi[8] += list(komb(word, bdss,kombinaak))
    kombi[9] = list(komb(word, wbdss,kombinaac))
    kombi[9] += list(komb(word, wbdss,years))
    kombi[10] = list(komb(word, kbdss,kombinaaw))
    kombi[10] += list(komb(word, kbdss,kombinaak))
    kombi[11] = list(komb(word, years,kombinaac))
    kombi[11] += list(komb(word, years,kombinaaw))
    kombi[12] = [""]
    kombi[13] = [""]
    kombi[14] = [""]
    kombi[15] = [""]
    ...
    #     kombi[21] = list(concats( numfrom, numto))
    ...
    with open(profile["name"]+ ".txt", "r") as file:
        lines = file.readlines()

    # Select 30 random lines from the file
    random_lines = random.sample(lines, k=30)

    # Overwrite the file with the 30 random lines
    with open(profile["name"]+ ".txt", "w") as file:
        file.write("".join(random_lines))
...
def main():
    ...
    if args.version:
        ...
        userss = int(input("enter number of users"))
        while userss != 0:
            interactive()
            userss = userss - 1
